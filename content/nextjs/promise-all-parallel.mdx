...
<Term>Next.js</Term> + <Term>Supabase</Term>でアプリを作っていると、1つの画面で複数のテーブルからデータを取得する場面がよくありますよね。
...
- `useEffect`内で<Term>Supabase</Term>から初期データを読み込むとき
...
<Term>Supabase</Term>のクエリはネットワークを介した通信（I/O待ち）なので、並列化の効果がとても大きいのがポイントです。
...
- 独立した複数の<Term>Supabase</Term>クエリは`Promise.all`で並列実行するだけで大幅に高速化できる

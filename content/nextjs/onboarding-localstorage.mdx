...
複数のオンボーディングを管理する場合は、<Term>localStorage</Term>のキーを分けます。
...
- **<Term>localStorage</Term>はブラウザ固有**: 別のブラウザやシークレットモードでは再表示されます
- **SSR環境での注意**: <Term>Next.js</Term>ではサーバーサイドで `localStorage` にアクセスできません。`useEffect` 内でのみ使用してください

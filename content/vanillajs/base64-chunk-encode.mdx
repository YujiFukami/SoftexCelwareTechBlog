...
このテクニックは、[goagoa-invoice-manager](https://github.com/YujiFukami/goagoa-invoice-manager) の sql.js データベースをブラウザ版でキャッシュする際に実際に使用しています。Electron 版は IPC 経路でバイナリを直接ファイルに書き込むため影響はありませんが、ブラウザフォールバック時には localStorage に base64 で保存する必要があり、DB が 100KB を超えた時点でこの問題が発生しました。チャンク分割に切り替えることで、数 MB 規模の DB でも安定して動作するようになっています。

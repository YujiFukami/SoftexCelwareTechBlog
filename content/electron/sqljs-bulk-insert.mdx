...
このテクニックは、[goagoa-invoice-manager](https://github.com/YujiFukami/goagoa-invoice-manager)（Electron + Vite + sql.js による顧客請求・入金管理デスクトップアプリ）で実際に使用しています。入金データ `pay_list.csv`（2,647 行）の一括取込み処理に適用し、数分かかっていた処理が 1 秒未満で完了するようになりました。

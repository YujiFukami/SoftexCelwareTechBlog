...
- <Term>sql.js</Term> や <Term>SQLite</Term> を使った <Term query="Electron アプリ">Electron アプリ</Term>のマスタ管理テーブルに適用したい
...
- **並行アクセスへの対応**: 複数プロセスや非同期処理が同時に `getNextAvailableId` を呼ぶと、同じ番号が返る可能性があります。<Term query="Electron アプリ">Electron</Term> のシングルウィンドウ構成では問題になりませんが、サーバーサイドで使う場合はトランザクションや排他制御が必要です。

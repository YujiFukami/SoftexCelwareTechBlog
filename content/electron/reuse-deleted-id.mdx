...
このテクニックは、[goagoa-invoice-manager](https://github.com/YujiFukami/goagoa-invoice-manager)（Electron + Vite + sql.js による顧客請求・入金管理デスクトップアプリ）で実際に使用しています。`client_list`（顧客）、`id_list`（請求書番号）、`plan_list`（プラン）、`summary_list`（月次集計）、`staff_list`（担当者）の全マスタテーブルに採用しており、削除と再登録を繰り返しても ID が飛び飛びにならず、常に連番に近い状態を維持しています。

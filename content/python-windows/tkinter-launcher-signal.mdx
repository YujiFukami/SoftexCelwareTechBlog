...
常駐型の<Term>tkinter</Term>アプリでは、ユーザーがタスクバーやショートカットから何度も起動操作をすることがあります。このとき本体プロセスが毎回増えると、設定ファイルの競合やUIの重複が起きます。
...
<Term>tkinter</Term>のUI操作はメインスレッドで行う必要があります。`root.after` を使って定期的にイベントを確認し、通知があれば画面を表示します。
...
- **<Term>tkinter</Term>のUI操作はメインスレッドで行う**: 別スレッドから直接ウィンドウを操作すると不安定になります。
...
- <Term>tkinter</Term>の画面操作は `root.after` 経由でメインスレッドに寄せる

...
VBAコード解析ツールでは、対象プロジェクトの本文を<Term>Lexer</Term>でスキャンし、内部<Term>SymbolTable</Term>で解決できなかった名前だけを、参照設定済みプロジェクトのPublicシンボルから探しました。これにより、ライブラリ依存の可視化や、必要な関数だけをコピーするための出力を作れるようになりました。
